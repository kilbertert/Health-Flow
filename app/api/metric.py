...
def get_metric_trend(
    ...
    with mysql_client.get_session() as db:
        ...
        reports = (
            ...
            .filter(ReportModel.patient_id == patient_id, ReportModel.created_at >= start_date)
            .order_by(ReportModel.created_at)
            .all()
        )

        if not reports:
            return {"patient_id": patient_id, "metric_name": metric_name, "data_points": [], "summary": "暂无数据"}
        ...
        metrics = (
            ...
            .filter(MetricModel.report_id.in_(report_ids), MetricModel.metric_name == metric_name)
            .all()
        )
                ...
                data_points.append(
                    {
                        ...
                        if report.exam_date
                        else report.created_at.isoformat(),
...
def get_anomalies(patient_id: str, days: int = Query(30, ge=1, description="查询天数范围")):
    """
    获取患者异常指标汇总。
    ...
    with mysql_client.get_session() as db:
        ...
        # 查询所有指标
        metrics = db.query(MetricModel).filter(MetricModel.report_id.in_(report_ids)).all()
        ...
        return {"anomalies": anomalies, "summary": f"发现{len(anomalies)}项异常指标，{len(metric_groups)}项不同指标"}
